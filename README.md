...
> 该数据集时间戳跨度约 294 天，但数据分布极度不均衡：99.96% 的记录集中在 2017-11-25 至 2017-12-03 的 9 天内，其余 75 天的记录仅为零星测试数据（单日不足 500 条）。这意味着**有效分析窗口实际只有约 9 天**，无法观察月度季节性，且 D7+ 留存存在右删失。在此约束下，分析聚焦于高时间分辨率（秒级）的行为序列建模；同时 2,900 万行的规模为验证 Polars / DuckDB 等现代工具在大数据量下的吞吐性能提供了充足的实验场。
