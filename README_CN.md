...
| A/B 实验为模拟 | 基于 user_id 奇偶分组替代真实实验 | 哈希随机分组 + SRM 检验 + CUPED 方差缩减 |
| 单机执行 | SQLite + 本地 CSV，无分布式查询 | Hive/Spark on 分区 Parquet + Airflow 调度 |

> A/B 测试模块实现了完整的统计流程（样本量计算 → 分组同质性检验 → 两比例 Z 检验 → Cohen's h → 95% 置信区间）。奇偶分组在无真实实验数据时是一种**有效的**随机化方案，统计方法论可直接迁移到生产实验平台。
