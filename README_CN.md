...
- **来源**：[阿里云天池 — 电商用户行为](https://tianchi.aliyun.com/dataset/649)
...
| 限制 | 影响 | 生产环境方案 |
|------|------|------------|
| 仅 10 天数据 | 无法观察月度/季度周期性，D7+ 留存右删失 | ≥90 天数据 + Prophet/ARIMA 时序预测 |
| 缺少金额字段 | 无法计算 GMV、客单价、ARPU，RFM 退化为 RF | 关联订单/交易表构建完整 RFM |
| 缺少用户属性 | 无性别、年龄、地域、设备等维度画像 | 关联用户画像表做分层分析 |
| A/B 实验为模拟 | 基于 user_id MD5 哈希分组替代真实实验 | 哈希随机分组 + SRM 检验 + CUPED 方差缩减 |
| 单机执行 | SQLite + 本地 CSV，无分布式查询 | Hive/Spark on 分区 Parquet + Airflow 调度 |

> A/B 测试模块实现了完整的统计流程（样本量计算 → 分组同质性检验 → 两比例 Z 检验 → Cohen's h → 95% 置信区间）。MD5 哈希分组在无真实实验数据时是一种**有效的**随机化方案，统计方法论可直接迁移到生产实验平台。
